...
#	}
  ...
  ##estimate sigma^2 to be given to the BART model
	...
	if (length(cov_prior_vec) != 0){
		...
		if (offset < 0){
			warning(paste("covariate prior vector length =", length(cov_prior_vec), "has to be equal to p =", ncol(model_matrix_training_data) - 1, "(the vector was lengthened and evenly distributed the weights)"))
			cov_prior_vec = c(cov_prior_vec, rep(1, -offset))
      cov_prior_vec = rep(1, length(cov_prior_vec)) / length(cov_prior_vec)
		}
