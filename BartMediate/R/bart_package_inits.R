...
## get gibbs output of cov_prior_select
get_cov_prior_select <- function(bart_machine) {
  check_serialization(bart_machine)

  num_burn <- bart_machine$num_burn_in

  out <- t(sapply(.jcall(bart_machine$java_bart_machine, "[[D", "getCovSplitProps"), .jevalArray))

  return(out[-(1:num_burn), ])
}
