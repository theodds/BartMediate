mediate_bart_stratify <- function(fit_y_0, fit_y_1, fit_m_0, fit_m_1,
                                  design_y, design_m,
                                  trt_name, med_name,
                                  max_iter = NULL,
                                  iters = NULL,
  ...
  N <- nrow(design_y)
  pred_type_y <- fit_y_0$pred_type
  pred_type_m <- fit_m_0$pred_type
  ...
  design_y_00 <- design_y
  design_y_01 <- design_y
  design_y_10 <- design_y
  design_y_11 <- design_y
  
  sigma_m_0 <- sigma_m_1 <- sigma_y_0 <- sigma_y_1 <- NULL
  if(pred_type_y == "regression") {sigma_y_0 <- sqrt(get_sigsqs(fit_y_0)); sigma_y_1 <- sqrt(get_sigsqs(fit_y_1))}
  if(pred_type_m == "regression") {sigma_m_0 <- sqrt(get_sigsqs(fit_m_0)); sigma_m_1 <- sqrt(get_sigsqs(fit_m_1))}
  ...
  for(j in 1:n_iter) {
    ...
    for(k in 1:num_copy) {
      mu_m_0  <- predict_iteration(fit_m_0, design_m, i)
      mu_m_1  <- predict_iteration(fit_m_1, design_m, i)
      
      epsilon <- rnorm(N)
      if(pred_type_m == "regression") {
        M_0 <- mu_m_0 + sigma_m_0[i] * epsilon
        M_1 <- mu_m_1 + sigma_m_1[i] * epsilon
      ...
      epsilon_y <- rnorm(N)
      mu_y_00   <- predict_iteration(fit_y_0, design_y_00, i)
      mu_y_01   <- predict_iteration(fit_y_0, design_y_01, i)
      mu_y_10   <- predict_iteration(fit_y_1, design_y_10, i)
      mu_y_11   <- predict_iteration(fit_y_1, design_y_11, i)
      if(pred_type_y == "regression") {
        Y_00      <- mu_y_00 + sigma_y_0[i] * epsilon_y
        Y_01      <- mu_y_01 + sigma_y_0[i] * epsilon_y
        Y_10      <- mu_y_10 + sigma_y_1[i] * epsilon_y
        Y_11      <- mu_y_11 + sigma_y_1[i] * epsilon_y
      ...
      out_list[[length(out_list) + 1]] <-
        data.frame(
          Iteration = i,
          CopyID    = k,
          Param     = effect(Y_00, Y_01, Y_10, Y_11, omega),
